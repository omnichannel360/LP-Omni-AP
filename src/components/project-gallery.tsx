...
export default function ProjectGallery({ title, summary, images }: ProjectGalleryProps) {
  ...
  return (
    <>
      {/* Banner Slider */}
      {hasImages && (
        <div className="relative w-full aspect-[16/9] sm:aspect-[21/9] bg-[#0a0a0a] overflow-hidden group">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 lg:p-10">
            ...
            {summary && (
              <p className="mt-1 sm:mt-2 text-xs sm:text-sm text-white/70 max-w-2xl line-clamp-2">{summary}</p>
            )}
          </div>
          ...
          {images.length > 1 && (
            <>
              <button
                onClick={prevBanner}
                className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-black/50 backdrop-blur-sm text-white flex items-center justify-center opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity hover:bg-black/70"
              >
              ...
              <button
                onClick={nextBanner}
                className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-black/50 backdrop-blur-sm text-white flex items-center justify-center opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity hover:bg-black/70"
              >
                <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {/* Image counter */}
          {images.length > 1 && (
            <div className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-black/50 backdrop-blur-sm rounded-full px-3 py-1 text-xs text-white/80">
      ...
      {!hasImages && (
        <div className="relative w-full aspect-[16/9] sm:aspect-[21/9] bg-gradient-to-br from-[#1a1a1a] to-[#333] flex items-end">
          <div className="p-4 sm:p-6 lg:p-10">
            <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-white">{title}</h1>
            {summary && <p className="mt-1 sm:mt-2 text-xs sm:text-sm text-white/70 max-w-2xl">{summary}</p>}
          </div>
        </div>
      )}

      {/* Image Tiles Grid — responsive: 2 cols mobile, 3 cols tablet, 4 cols desktop */}
      {hasImages && (
        <div className="px-3 sm:px-6 lg:px-10 py-1 max-w-[1200px] mx-auto">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-[3px]">
          ...
          {images.length > 8 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-[3px] mt-[3px]">
